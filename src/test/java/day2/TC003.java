...
public class TC003 {
   ...
   public String body;
    ...
    public void addContact() throws FileNotFoundException {
        ...
        body = ReadYml.getJsonStringFromYmal("add_contact_json");
        body =  ReadYml.setBodyValue(body,"email","monika@test.com");
        body= ReadYml.setBodyValue(body,"phone","4232423222");
        body= ReadYml.setBodyValue(body,"firstName","monika");

        Response response = given().contentType(ContentType.JSON).header("Authorization","Bearer "+token).log().all().body(body).post();
